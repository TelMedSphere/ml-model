...
//     const data = {
//       symptoms: this.state.user_symptoms,
//     };
...
//         // this.sendSymptomsToBackend(); // Send data after updating
//       });
//     }
//   };
...
const Symptom = forwardRef((props, ref) => {
  ...
  // Expose the sendSymptomsToBackend method to the parent
  ...
  const addSymptom = (symptom) => {
    if (!userSymptoms.includes(symptom)) {
      setUserSymptoms([symptom, ...userSymptoms]);
    }
  };
  ...
  return (
    <div>
      <input
        ...
        placeholder="Search Symptoms"
      />
      <button onClick={sendSymptomsToBackend}>Send Symptoms</button>
      <ul>
        {Symptoms.filter((symptom) =>
          symptom.toLowerCase().includes(searched.toLowerCase())
        ).map((symptom) => (
          <li key={symptom}>
            <button onClick={() => addSymptom(symptom)}>{symptom}</button>
          </li>
        ))}
      </ul>
      <ul>
